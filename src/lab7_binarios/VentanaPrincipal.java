...
public class VentanaPrincipal extends JFrame implements ReproductorMusica.ReproductorListener {
    ...
    public VentanaPrincipal() {
        ...
        gestor      = new GestorCancionesArchivo("data/canciones.dat");
    ...
    private JPanel construirFormulario() {
        ...
        JButton btnAudio = crearBtnPequeno("...");
        btnAudio.setToolTipText("Buscar archivo de audio");
        btnAudio.addActionListener(e -> seleccionarArchivoAudio());
        p.add(crearFilaConBtn("Archivo de audio", txtRutaAudio, btnAudio));
        ...
        txtDuracion = crearCampo("Detectado automaticamente");
        txtDuracion.setEditable(false);
        p.add(crearFila("Duración (seg) — automática", txtDuracion));
        p.add(Box.createVerticalStrut(8));

        txtRutaImagen = crearCampo("Opcional");
        txtRutaImagen.setEditable(false);
        JButton btnImg = crearBtnPequeno("...");
        btnImg.setToolTipText("Buscar imagen del album");
        btnImg.addActionListener(e -> seleccionarImagen());
        p.add(crearFilaConBtn("Imagen del album", txtRutaImagen, btnImg));
        p.add(Box.createVerticalStrut(8));
    ...
    private JPanel construirCentro() {
        ...
        lblNombre  = centeredLabel("Selecciona una canción", new Font("SansSerif", Font.BOLD, 24), TEXT_WHITE);
        lblArtista = centeredLabel(" ", new Font("SansSerif", Font.PLAIN, 16), TEXT_GRAY);
        lblGenero  = centeredLabel(" ", new Font("SansSerif", Font.PLAIN, 13), TEXT_DIM);
        np.add(lblNombre);
    ...
    private JPanel crearFilaCancion(Cancion c, int idx) {
        ...
        fila.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { if (!activa) fila.setBackground(BG_HOVER); fila.repaint(); }
            @Override public void mouseExited (MouseEvent e) { if (!activa) fila.setBackground(BG_BASE);  fila.repaint(); }
            @Override public void mouseClicked(MouseEvent e) {
                seleccionar(idx);
                if (e.getClickCount() == 2) accionPlay();
            }
        });
        return fila;
    }
    ...
    private void accionPlay() {
        if (idxSel < 0 || idxSel >= canciones.size()) { feedback("Selecciona una canción primero", false); return; }
    ...
    private void accionPause() {
        if      (reproductor.estaReproduciendo()) reproductor.pause();
        else if (reproductor.estaPausado())        reproductor.resume();
    }

    private void accionStop() { reproductor.stop(); }

    private void seleccionar(int idx) {
        idxSel = idx;
        refreshLista();
        if (idx >= 0 && idx < canciones.size()) mostrarInfoCancion(canciones.get(idx));
    }

    private void toggleFormulario() {
        formVisible = !formVisible;
        panelForm.setVisible(formVisible);
        if (formVisible) limpiarFormulario();
    ...
    private void seleccionarArchivoAudio() {
        ...
        fc.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter(
            "Archivos de audio (*.wav, *.mp3, *.aiff, *.au)", "wav", "mp3", "aiff", "au"));
        ...
        if (txtNombre.getText().trim().isEmpty()) {
            ...
            txtNombre.setText(dot > 0 ? nombre.substring(0, dot) : nombre);
        }
        ...
        if (dur > 0) {
            ...
            txtDuracion.setText("No detectado");
            txtDuracion.setForeground(ERR);
            feedback("No se pudo detectar la duración", false);
        }
    }
    ...
    private void seleccionarImagen() {
        ...
        cargarArtwork(fc.getSelectedFile().getAbsolutePath());
    }
    ...
    private void guardarCancion() {
        ...
        if (nombre.isEmpty())  { feedback("El nombre es requerido",           false); return; }
        if (artista.isEmpty()) { feedback("El artista es requerido",          false); return; }
        if (rutaAud.isEmpty()) { feedback("Selecciona un archivo de audio",   false); return; }
        ...
        try {
            ...
            feedback("Duración inválida — selecciona el archivo de audio", false);
            return;
        }

        Cancion nueva = new Cancion(nombre, artista, duracion, rutaImg, genero);
        try {
    ...
    private void eliminarSeleccionada() {
        if (idxSel < 0) { feedback("Selecciona una canción para eliminar", false); return; }
        Cancion c = canciones.get(idxSel);
        if (!reproductor.estaDetenido()) {
            Cancion actual = reproductor.getCancionActual();
            if (actual != null && actual.equals(c)) reproductor.stop();
        }
        ...
        if (canciones.isEmpty()) {
            ...
            for (int i = 0; i < canciones.size(); i++) {
                ...
                if (i < canciones.size() - 1) {
                    ...
                    sep.setForeground(SEP); sep.setBackground(SEP);
    ...
    private void limpiarFormulario() {
        txtNombre.setText(""); txtArtista.setText("");
        txtDuracion.setText(""); txtDuracion.setForeground(TEXT_DIM);
        txtRutaAudio.setText(""); txtRutaImagen.setText("");
    ...
    private void resetearInfoCancion() {
        ...
        lblArtista.setText(" "); lblGenero.setText(" ");
        lblTiempoActual.setText("0:00"); lblTiempoTotal.setText("0:00");
        progressBar.setValue(0);
        defaultArtwork();
    }

    private void cargarArtwork(String ruta) {
        if (ruta == null || ruta.trim().isEmpty()) { defaultArtwork(); return; }
        try {
    ...
    public void onEstadoChanged(EstadoReproductor estado) {
        SwingUtilities.invokeLater(() -> {
            ...
            switch (estado) {
                case PLAYING: lblEstado.setForeground(ACCENT); break;
                case PAUSED:  lblEstado.setForeground(new Color(255, 200, 60)); break;
                case STOPPED:
                    lblEstado.setForeground(TEXT_DIM);
                    progressBar.setValue(0);
                    lblTiempoActual.setText("0:00");
    ...
    private JLabel centeredLabel(String txt, Font f, Color c) {
        ...
        l.setFont(f); l.setForeground(c); l.setAlignmentX(CENTER_ALIGNMENT);
        return l;
    }
    ...
    private JTextField crearCampo(String hint) {
        ...
        tf.setBackground(BG_ELEVATED); tf.setForeground(TEXT_WHITE); tf.setCaretColor(ACCENT);
    ...
    private JComboBox<GeneroMusical> crearCombo() {
        ...
        cb.setBackground(BG_ELEVATED); cb.setForeground(TEXT_WHITE);
        cb.setFont(new Font("SansSerif", Font.PLAIN, 13));
        cb.setBorder(new LineBorder(new Color(60, 60, 60), 1, true));
        cb.setRenderer(new DefaultListCellRenderer() {
            @Override public Component getListCellRendererComponent(JList<?> l, Object v, int i, boolean s, boolean f) {
                super.getListCellRendererComponent(l,v,i,s,f);
                setBackground(s ? BG_HOVER : BG_ELEVATED); setForeground(TEXT_WHITE);
                setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8));
                if (v instanceof GeneroMusical) setText(((GeneroMusical)v).getDescripcion());
                return this;
            }
        });
        return cb;
    ...
    private JPanel crearFila(String lbl, JTextField tf) {
        ...
        p.setOpaque(false); p.setMaximumSize(new Dimension(278, 56)); p.setAlignmentX(LEFT_ALIGNMENT);
        JLabel l = new JLabel(lbl); l.setFont(new Font("SansSerif", Font.PLAIN, 11)); l.setForeground(TEXT_DIM);
    ...
    private JPanel crearFilaConBtn(String lbl, JTextField tf, JButton btn) {
        ...
        outer.setOpaque(false); outer.setMaximumSize(new Dimension(278, 56)); outer.setAlignmentX(LEFT_ALIGNMENT);
        JLabel l = new JLabel(lbl); l.setFont(new Font("SansSerif", Font.PLAIN, 11)); l.setForeground(TEXT_DIM);
        outer.add(l, BorderLayout.NORTH);
        JPanel row = new JPanel(new BorderLayout(4, 0));
        row.setOpaque(false); row.add(tf, BorderLayout.CENTER); row.add(btn, BorderLayout.EAST);
    ...
    private JButton crearBtnPequeno(String txt) {
    ...
    private JButton crearBtnPri(String txt) {
        ...
        b.setPreferredSize(new Dimension(88,33)); b.setFocusPainted(false);
    ...
    private String fmtTiempo(int s) { return (s/60) + ":" + (s%60<10?"0":"") + (s%60); }
    private String trunc(String s, int n) { return s.length()>n ? s.substring(0,n-1)+"\u2026" : s; }
    ...
    static class CtrlButton extends JButton {
        ...
        CtrlButton(int tipo, int sz, boolean primario) {
            this.tipo = tipo; this.sz = sz; this.primario = primario;
        ...
        protected void paintComponent(Graphics g) {
            ...
            switch (tipo) {
                ...
                case PAUSE: {
                    int bw  = Math.max(3, arm/2);
                    int bh  = arm + arm/2;
                    int gap = Math.max(2, arm/3);
    ...
    static class IconBtn extends JButton {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.drawLine(cx - arm, cy, cx + arm, cy);          // horizontal
            if (plus) g2.drawLine(cx, cy - arm, cx, cy + arm); // vertical
            g2.dispose();
        }
    }
    ...
    static class DarkScrollUI extends javax.swing.plaf.basic.BasicScrollBarUI {
        @Override protected void configureScrollBarColors() { thumbColor = BG_ELEVATED; trackColor = BG_BASE; }
    ...
    static class SlimProgressUI extends javax.swing.plaf.basic.BasicProgressBarUI {
        @Override public void paint(Graphics g, JComponent c) {
            ...
            if (filled > 0) { g2.setColor(ACCENT); g2.fillRoundRect(0, 0, filled, h, h, h); }
            g2.dispose();
        }
        @Override public Dimension getPreferredSize(JComponent c) { return new Dimension(340, 4); }
    }
